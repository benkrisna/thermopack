...
contains
  ...
  !> Get energy constant
  subroutine get_acentric_factor(omega)
    use thermopack_var
    implicit none
    real, intent(out) :: omega(nce) ! Energy constant in front of alpha. Units: Pa*L^2/mol^2.
    ! Locals
    class(base_eos_param), pointer :: act_eos_ptr
    !
    act_eos_ptr => get_active_eos()
    ...
    select type(p_eos => act_eos_ptr)
    class is (cb_eos)
      omega = p_eos%single(:)%acf
    class default
      print *,"get_acentric_factor: Wrong model - not cubic"
    end select
  end subroutine get_acentric_factor
